...
parser.add_option("-p", "--policy", default="RE", help="sched policy to use: SJF, FIFO, RR, RE",
                  action="store", type="string", dest="policy")
parser.add_option("-q", "--quantum", help="length of time slice for RR policy", default=1, 
                  action="store", type="int", dest="quantum")
parser.add_option("-r", help="the list of required resource: 2(using), 1(required), 0(not required)", action="store", default="", type="string", dest="resource")
parser.add_option("-v", help="the level of priority", action="store", default="", type="string", dest="priority")
...
if options.jlist == '':
    if options.policy == 'RE':
        for jobnum in range(0,options.jobs):
            ...
            resource.append(random.randint(0, 3))
            priority.append(random.randint(0, options.jobs + 1))
    else:
        for jobnum in range(0,options.jobs):
            runtime = int(options.maxlen * random.random()) + 1
            joblist.append([jobnum, runtime])
...
for i in xrange(len(joblist)):
    if options.policy == 'RE':
        print '  Job', i, '( length = ' + str(joblist[i][1]) + ' ), ',  'priority=', priority[i], ', required resource=', resource[i]
    else:
        print '  Job', i, '( length = ' + str(job[1]) + ' )'
print '\n'
...
if options.solve == True:
    ...
    if options.policy != 'FIFO' and options.policy != 'SJF' and options.policy != 'RR' and options.policy != 'RE': 
