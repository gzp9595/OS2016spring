...
parser.add_option("-r", help="the list of required resource: 2(using), 1(required), 0(not required)", action="store", default="3,2,1", type="string", dest="resource")
parser.add_option("-v", help="the level of priority", action="store", default="3,2,1,0", type="string", dest="priority")
...
if options.jlist == '':
    if options.policy == 'RE':
        for jobnum in range(0,options.jobs):
            ...
            print '  Job', jobnum, '( length = ' + str(runtime) + ' ), ',  'priority=', priority[jobnum], ', required resource=', resource[jobnum]
    else:
        for jobnum in range(0,options.jobs):
            runtime = int(options.maxlen * random.random()) + 1
            joblist.append([jobnum, runtime])
...
if options.solve == True:
    ...
    if options.policy != 'FIFO' and options.policy != 'RE' and options.policy != 'RR': 
